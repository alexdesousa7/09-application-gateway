#v0_1
resource "azurerm_virtual_machine_scale_set" "testag" {
  ...
  os_profile {
    computer_name_prefix = "testagvm01"
  ...
  network_profile {
    name                                     = "networkprofile"
    primary                                  = true
    network_security_group_id                = azurerm_network_security_group.testag-instance.id

    ip_configuration {
      name                                   = "IPConfiguration"
      primary                                = true
      subnet_id                              = azurerm_subnet.testag-subnet-2.id
